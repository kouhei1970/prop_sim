...
## 8. 実測との突き合わせ (推力試験 1 点) — **較正は未適用**

> **現時点で較正は行っていない。** まとまった推力-回転数の試験結果がまだ
> ないため、`stampfly_1209()` プリセットは**未較正のまま**である。
> 以下は「いま手元にある 1 点と比べるとどうか」の記録であり、モデルを
> この点に合わせ込んだものではない。
...
それまでは**較正を適用しない**方針とする。1 点だけで合わせ込むと、
どちらの仮説を選んだかによってホバー予測が 23 % ずれ、しかもその不定性が
モデルの中に隠れてしまうため。較正機能 (`prop_sim.calibration`) は
データが揃った時点で下記のように使う。
...
## 9. メーカ公表データ (Gemfan 1209-4, 3.7 V) との突き合わせ

Gemfan 公式の "Test Datas" シート (`prop_sim.data.GEMFAN_1209_4`)。
スロットル 10〜100 % に対する 電流 / 推力 / 推力効率。

**回転数が公表されていないため、これだけでは空力モデルを較正できない。**
推力もトルクも回転数の関数であり、電気入力からはモータ効率と分離できない。

### データ自体の内部矛盾

推力列と「T.E.1 × 電気入力」が一致しない。

| スロットル % | 10 | 30 | 50 | 60 | 80 | 100 |
|---|---|---|---|---|---|---|
| 推力列 [g] | 1.00 | 4.00 | 12.00 | 16.00 | 22.00 | 29.00 |
| T.E.1 × P [g] | 0.62 | 3.93 | 10.59 | 14.56 | 20.84 | 28.35 |
| ずれ | −38 % | −2 % | −12 % | −9 % | −5 % | −2 % |

推力列は整数に丸められている (1, 2, 4, 7, 12, …) ので、T.E.1 の方が元の
分解能を保っている可能性が高い。`gemfan_1209_4_points(source=...)` で
どちらを使うか選べる。

### 回転数がなくても言えること

**1. モデルは反証されない。** 各点で「モデルがその推力を出すのに必要な
回転数」を逆算し、そのときの軸動力から必要なモータ効率を求めると

| スロットル % | 10 | 30 | 50 | 70 | 90 | 100 |
|---|---|---|---|---|---|---|
| 逆算 rpm | 9,165 | 18,127 | 31,119 | 38,996 | 44,609 | 47,968 |
| 必要な η_motor | 0.044 | 0.094 | 0.233 | 0.321 | 0.366 | **0.371** |

**どこでも η < 1** かつ単調増加で、1S 微小モータとして妥当な値
(30〜45 %)。つまりこのデータではモデルを否定できない。

**2. 回転数によらない実測量**が 1 つ得られる。

```
FM × η_motor = P_ideal / P_elec = T^1.5 / (√(2ρA) · P_elec) = 0.264  (全開)
```

η ≤ 0.45 (微小モータの上限) を仮定すると **FM ≥ 0.59**。モデルの
FM = 0.71 と矛盾しない。

**3. 最大推力の実測値** — 3.7 V 全開で 29 g。これは効く数字。

### それでも仮説は切り分けられない

| 仮説 | 1 gf の rpm | 全開 (29 g) の rpm | FM | 必要 η_max |
|------|------------|-------------------|-----|-----------|
| 未較正 | 9,165 | 47,968 | 0.712 | 0.371 |
| 取付角 (−7.81°) | 11,545 | **60,792** | 0.648 | 0.408 |
| 低 Re (re_lift=1.02) | 11,545 | 47,631 | 0.712 | 0.371 |

3 つともモータ効率が妥当な範囲に収まってしまう。ただし**傾向は見える**:

- **低 Re 仮説は全開ではほとんど効かない** (47,631 vs 未較正 47,968、差 0.7 %)。
  Re が上がると補正が消えるため。
- **取付角仮説は全開で 60,800 rpm を要求する** — Kv ≥ 16,400 rpm/V が
  無負荷でも必要になる。3.7 V の 1S でこれはやや厳しい。

Gemfan の試験モータの Kv が分かれば、この差で決着する可能性がある。

### 結論

このデータは**反証テストと FM の下限**には使えるが、**較正には使えない**。
必要なのは回転数。ユーザ側で 1 点は測れている (1209 rad/s) ので、
**同じ測定をスロットル 4〜5 点で繰り返す**のが最短。

```python
from prop_sim.data import gemfan_1209_4_points
out = ps.consistency_check(rotor, ps.BEMT(), gemfan_1209_4_points())
out["falsified"], out["max_efficiency"], out["rpm"]
```
