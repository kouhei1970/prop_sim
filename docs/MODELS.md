...
計算コストの順に整理する。**Lv0〜Lv3 と LvS を実装済み**、Lv4 は 2 次元翼型
CFD (OpenFOAM) を実装し、3 次元 CFD は外部の結果を取り込む口を用意してある。
...
| 4 | CFD (2 次元翼型) | `prop_sim.cfd` (OpenFOAM) | ~10-30 分/迎角 | — | 低 Re 翼型ポーラの決定 |
| 4' | CFD (外部 3 次元) | `SurrogateModel.from_conditions` | ~0.2 ms | データ次第 | 外部 CFD/実測の取り込み |
...
### Lv3 — 揚力線 + 渦後流 (`LiftingLine`)

ブレードを束縛渦、後流を規定螺旋渦として Biot–Savart で誘導速度を計算する。
BEMT のアニュラス独立仮定と Prandtl 損失を置き換えられ、後流の旋回も入る。
**軸流のみ**（斜め流入は非対応）。上の表の誤差分解はこのモデルで取った。

自由渦後流 (FVW) は実装していない。素朴な緩和法では数巻きで後流が絡まり、
z/R が −3.9 まで伸びずに −0.3 で止まってしまった。まともにやるには
Bagai–Leishman の PC2B 法と渦核成長モデルが要る。

### Lv4 — CFD

**2 次元翼型 CFD は `prop_sim.cfd` に実装済み**（OpenFOAM）。
断面形状 → `cl(alpha, Re)` / `cd(alpha, Re)` → `TabulatedAirfoil` の経路で、
`airfoil_from_section()` の低 Re 経験補正を物理計算に置き換えられる。
円柱 Re = 20 / 40 の既知解で検証してある → [docs/CFD.md](CFD.md)

3 次元ロータ CFD（滑りメッシュ RANS、アクチュエータライン LES）は
数百万セル・数時間〜数日なので本パッケージでは回さない。実務では
「BEMT で広範囲を掃引 → 数点だけ CFD で検証 → 差分を補正係数にする」が
現実的で、外部で回した 6 分力は `SurrogateModel.from_conditions()` で
取り込める（試験シナリオ・センサモデルはそのまま流用できる）。
